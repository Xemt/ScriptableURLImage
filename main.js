...
const URLImage = new Proxy({
 image: null,
 url: null,
 get: async function(a){if(new.target)throw"Do not call this function using 'new'";else{if(a&&typeof a=="string"&&a.toLowerCase().match(/^(http(s)?:\/\/|data:image\/(aces|apng|avif|bmp|gif|heic|heic-sequence|heif|heif-sequence|jpeg|png|svg\+xml|tiff|tiff-fx)|blob:(\/\/|\/\/\/)?|file:\/\/)/g)){this.url=a;await new Request(a).loadImage().then(b=>{this.image=b},c=>{throw c});return this.image}else{throw"URL is invalid"}}}, // get
